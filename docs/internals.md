...
## Mask compilation (`Route::parseMask`, ~450)
...
- **Fixity is a three-state private constant** (`InQuery=0`, `InPath=1`,
  `Constant=2`, ~60) that controls matching precedence and overridability.
  `Constant` = value given in metadata but **not** present in the path mask, so
  it can never be changed by URL or query. `InPath` = optional parameter with a
  default. This state, not the presence of a default alone, is what later code
  branches on.
...
- **A per-parameter `FilterIn` returning `null` rejects the match only for
  parameters without fixity.** For a parameter with a default, the `null`
  survives into the result instead.
...
## URL generation (`Route::constructUrl` ~253, `preprocessParams` ~300, `compileUrl` ~356)
...
- **Non-scalar values for masked params are not guarded.** `preprocessParams`
  string-casts scalars (`false` → `'0'`), but an array/object supplied for a
  path parameter falls through to the default `FilterOut` (`param2path`, typed
  `string`) and throws `TypeError`. Non-scalars are safe only as query
  parameters, where they end up in `http_build_query`.
...
- **Pattern is re-checked on the way out.** After filters, the value must still
  match the parameter's compiled `Pattern` (tested against the `rawurldecode`d
  value), otherwise generation returns `null`.
...
- **Generation index (`warmupCache`, ~144).** Routers are grouped by the value
  ...
  - Only `Route` instances contribute constant parameters
    (`getConstantParameters`); any other router — including nested
    `RouteList`s — exposes none and is therefore added to **every** bucket
    (`$value === null` → `array_keys($ranks)`).
  - **Routes added with the `$oneWay` flag are excluded from the index
    entirely** (`Router::ONE_WAY` is its deprecated constant form), so they
    never generate URLs — this is the mechanism behind canonization: the first
...
| Match precedence, filters | `Route::match` |
| URL build, auto-optional, constant gate | `Route::constructUrl`, `preprocessParams`, `compileUrl` |
| Host wildcard substitution | the `strtr` tables in `Route::match`/`constructUrl`, `RouteList::expandDomain` |
